...
## Aims of this training {.smaller}
- Build your confidence in using command line Linux.
- Prepare you for [HPC1: Introduction to High Performance Computing](https://arc.leeds.ac.uk/training/courses/hpc1/) (HPC1).
- Get feedback from you about the training.

::: {.notes}
- Attempt to use Forms form or Hackpad for introductions.
...
::: {.notes}
- Your local terminal provides something like the command line Linux on ARC3/ARC4
interface program.
- `MobaXTerm` is software you can download, install and run without needing help from systems administators.
...
- A shell is a terminal emulator program an scripting language.
- The terminal is a window with a prompt at which textual commands are entered.
- Standard Output (stdout) and Standard Error (stderr) streams get displayed in the window.
- During login, your set up is finalised by reading some files that reside in your Home Directory.
- When logged in you will be in your Home Directory.


## What is BASH? {.smaller}
- A type of shell that for example looks like:
...
- Shells call other programs by name, and these are found via the users `PATH`:
  - A variable and ordered list of filesystem directories.
...
::: {.notes}
- Details of the module system and how to use it are covered in HPC1.
- Details of installing software is covered in HPC2.
:::
...
- The image shows a simplified filesystem.
  - We will look at the actual file system shortly...
...
- The `~` (tilde) in the prompt indicates that you are in your Home Directory.
- The `$` (dollar) symbol is the end of the prompt and is where commands get typed/entered.
...
- Hints:
  ...
  - Direct the ouput into a file and then use `nano` to modify this into a script.
...
::: {.notes}
- Scripts you write that you commonly want to run can be put somehwere sensible and this can be added to your `PATH` variable.
