...
```
- Moving an important hidden file or directory could be bad!
- A move can usually be reversed, but once things are moved around they are more easily lost!
...
- The `cat` command will concatenate and print to standard output.
  - This can be used to stream one file after another.
...
- Commands that create an output stream can have the output piped into another command that can process the stream using the symbol '`|`'. 
  - For example two files '`test`' and '`test2`' can be catenated, sorted alphabetically and piped into head as follows: 
```{.bash code-line-numbers=false}
[user@host ~]$ cat test test2 | sort | head
```


## `cat`, `sort` and `cut` demo
...
## `sed` 1/2 {.smaller}
- `sed` is a program command tool for stream editing. 
...
## `sed` 2/2 {.smaller}
- `sed` is complicated and powerful, but less so that `awk`.
...
## Visibility and Permissions 5/6
...
## Visibility and Permissions 6/6
...
- `i` is initialised with a value of `1`; the loop continues until the condition '`i<6`'; each iteration of the loop `i` is incremented by `2`.


## for loop exercise {.smaller}
...
## sequence exercise {.smaller}

::: {.panel-tabset}

### Problem
- Use a sequence to create five directories, each one the parent of the next.
...
```
- Two commands are executed for each item in the sequence.
